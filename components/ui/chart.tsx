...
const ChartTooltipContent = React.forwardRef<
  ...
  Omit<React.ComponentProps<typeof RechartsPrimitive.Tooltip>, "payload"> & {
    ...
    formatter?: (value: number, name: string, props: TooltipPayload) => React.ReactNode
    color?: string
    label?: React.ReactNode
  }
>(
              ...
              >
                {!hideIndicator && (
                  <>
                    {indicator === "dot" && (
                      <span
                        className="h-2.5 w-2.5 translate-y-[2px] rounded-full"
                    ...
                    {indicator === "dashed" && (
                      <span
                        className="h-full w-0.5 border-l-2 border-dashed"
...
>(({ className, payload, verticalAlign, hideIcon, nameKey, ...props }, ref) => {
  ...
  return (
    ...
    >
      {payload?.map((entry) => {
        const key = `${nameKey || entry.value || "value"}`
